...
from src.mgmt.tests.common.arion_api import ArionApi
...
class test_arion_aca_ping(unittest.TestCase):

    def setUp(self):
        self.arion_api = ArionApi()
        ...
        json_content_for_aca['vpc_response'] = self.vpc_response = self.arion_api.create_vpc(
            vpc).json()
        json_content_for_aca['port_response'] = self.ports_response = self.arion_api.create_port(
            ports).json()
        ...
        testcases_to_run = ['DISABLED_arion_scale_CHILD',
                            'DISABLED_arion_scale_PARENT']
