...
class ArionApi:
    ...
    def __init__(self):
        manager_ip_cmd = "kubectl get node $(kubectl get pods -o wide | grep arion-manager | awk '{print $7}' | cut -d/ -f1) -o wide | grep kind | awk '{print $6}' | cut -d/ -f1"
        self.ip = run_cmd(manager_ip_cmd)
        self.port = "80"
        self.address = "http://{}:{}".format(self.ip, self.port)

    def arion_http_request_json(self, verb, resource, data):
    ...
    def create_vpc(self, data):
        return self.arion_http_request_json("post", "vpcs", json.dumps(data))

    def create_port(self, data):
        return self.arion_http_request_json("post", "ports", json.dumps(data))
