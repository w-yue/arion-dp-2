...
# Remove existing arion-nodes containers
DROPLETS=$(docker ps -a --format "{{.Names}}" | grep arion-node)
echo "Deleting existing arion-node containers"
docker stop $DROPLETS >/dev/null 2>&1
docker rm -f $DROPLETS >/dev/null 2>&1
docker container prune -f >/dev/null 2>&1

if [[ ! -z "$STAGE" && "$STAGE" != "user" ]]; then
    echo "Rebuild and publish arion_droplet image to $REG..."
    docker image build -t $REG/arion_droplet:latest -f ${ROOT}/deploy/kind/droplet.Dockerfile $ROOT >/dev/null
    docker image push $REG/arion_droplet:latest >/dev/null
fi

# Bring up droplets
echo "Creating $DROPLET_NODES containers as ZGC nodes"
for ((i = 1; i <= $DROPLET_NODES; i++)); do
    echo -e "Creating arion-node-$i"
    docker run -d \
        --privileged \
        --cap-add=NET_ADMIN \
        --cap-add=SYS_PTRACE \
        ...
        --name arion-node-$i \
        $REG/arion_droplet:latest
done

# Populate default zgc for KIND deployment
manager_ip=$(kubectl get node $(kubectl get pods -o wide | grep arion-manager | awk '{print $7}' | cut -d/ -f1) -o wide | grep kind | awk '{print $6}' | cut -d/ -f1)
...
inf_zgc="eth1"
inf_tenant="eth2"
nodes=($(docker ps | grep -o 'arion-node-[0-9]\+'))
